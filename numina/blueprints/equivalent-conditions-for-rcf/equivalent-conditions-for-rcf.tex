% Blueprint: Equivalent Conditions for RCF
% Created by Artie Khovanov using leanblueprint software
...
\title{Real Closed Field}
\author{Artie Khovanov}
...
\begin{lemma}
  ...
  Let $F$ be an ordered field, and let $K/F$ be a field extension. Suppose that there is an $F$-linear functional $\pi:K\to F$ such that $\pi(1)=1$ and, for all $x\in K$, $\pi(x^2)\geq0$. Then there is a field ordering on $K$ making $K/F$ ordered.
\end{lemma}
\begin{proof}
