...
private lemma exists_ordered_algebra_adjoinRoot_odd_irreducible
  ...
  -- For each y in support (a square), pick its representative in R[X] of degree < g.natDegree
  -- Build the polynomial P := (∑ (c y).val • (rep y)^2) + 1, which is divisible by g.
  -- First, lift each y ∈ supp c to a polynomial p y with natDegree p y < g.natDegree.
  let p : K → R[X] := fun y => hm.modByMonicHom y
  have hp_deg : ∀ y : K, (p y).natDegree < g.natDegree ∨ p y = 0 := by
    intro y
    by_cases h0 : p y = 0
    · right; exact h0
    · left
      ...
      exact Polynomial.natDegree_modByMonic_lt (hm.repr y) hg_monic hg_ne_one
  -- For each y that's a square, get a witness
  -- y = (sq y)^2 where sq y is chosen via classical
  have hy_sq : ∀ y ∈ c.support, IsSquare y := by
    intro y hy
    exact hc_supp hy
  -- For each y ∈ c.support, y = (sqRoot y)^2
  ...
  have hsqRoot : ∀ y ∈ c.support, y = (sqRoot y)^2 := by
    ...
    rw [this]; ring
  -- Define q y := p (sqRoot y); the polynomial lift of the square root.
  let q : K → R[X] := fun y => p (sqRoot y)
  -- Now, in AdjoinRoot g: y = (AdjoinRoot.mk g (q y))^2 for y ∈ c.support
  have hy_mk : ∀ y ∈ c.support, y = (AdjoinRoot.mk g (q y))^2 := by
    intro y hy
    have h1 : AdjoinRoot.mk g (q y) = sqRoot y := by
      show AdjoinRoot.mk g (hm.modByMonicHom (sqRoot y)) = sqRoot y
      -- AdjoinRoot.mk g is the .map of hm.  The map is left-inverse of modByMonicHom.
      have := hm.map_modByMonicHom (sqRoot y)
      show hm.map (hm.modByMonicHom (sqRoot y)) = sqRoot y
      exact this
    rw [h1]; exact hsqRoot y hy
  -- Build the polynomial sum P := ∑ (c y).val • (q y)^2 + 1.
  set P : R[X] := (∑ y ∈ c.support, ((c y : R) : R[X]) * (q y)^2) + 1 with hP_def
  -- Goal: AdjoinRoot.mk g P = 0, i.e., g | P.
  have hmk_P_zero : AdjoinRoot.mk g P = 0 := by
    have hsum_eq : AdjoinRoot.mk g (∑ y ∈ c.support, ((c y : R) : R[X]) * (q y)^2) = -1 := by
      rw [show (-1 : K) = -(1 : K) from rfl, ← hc_sum]
      rw [map_sum]
      rw [show -∑ y ∈ c.support, (c y : R) • y = ∑ y ∈ c.support, -((c y : R) • y) from by
        rw [← Finset.sum_neg_distrib]]
      refine Finset.sum_congr rfl fun y hy => ?_
      rw [map_mul, AdjoinRoot.mk_C, map_pow]
      rw [show AdjoinRoot.mk g (q y) = sqRoot y from ?_]
      · rw [show ((sqRoot y) ^ 2 : K) = y from (hsqRoot y hy).symm]
        -- now: algebraMap R K ↑(c y) * y = -(↑(c y) • y)  -- wait this is wrong
        -- actually hc_sum is : ∑ y ∈ c.support, (c y) • y = -1
        -- so -1 = ∑ (c y) • y, and we want mk P - 1 to be... hmm
        sorry
      · have := hm.map_modByMonicHom (sqRoot y)
        exact this
    sorry
